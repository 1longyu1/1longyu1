...
def select(answer, key):  # 确定选项是否正确
    ...
    for line in file_temp:
        if answer in line:
            if "?" in line:
                try:
                    click_answer = driver.find_elements \
                        (By.CSS_SELECTOR, "ul.exes_option > li > input")  # 找到所有的选项
...
def click_ensure_submit():  # 提交
    try:
        click_submit = driver.find_element(By.CSS_SELECTOR, "#s2 > a")
        driver.execute_script("arguments[0].click();", click_submit)
        click_ensure = driver.find_element(By.CSS_SELECTOR, "body > div.aui_state_lock.aui_state_focus > div > "
...
if __name__ == '__main__':
    ...
    for i in range(2254, 2309):
        ...
        if flag == 1:
            topic_S = driver.find_elements(By.CSS_SELECTOR, "ul>li>p")
            # 找到所有的选项
            Option = driver.find_elements(By.CSS_SELECTOR, "ul.exes_option > li")
            # xt_12407 > ul > li:nth-child(1)
            # 判断哪些是单选题，哪些是多选题
            title = driver.find_elements(By.CSS_SELECTOR, "dd.bookExes_list>ul>li>h5")
            ...
            click_ensure_submit()
            chapter = driver.find_elements(By.CSS_SELECTOR,"dd>ul>li>a")  # 重新获取页面，新加载的页面
